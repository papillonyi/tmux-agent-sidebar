---
title: Activity log
description: Tool-call history rendered in the lower Activity panel.
---
...
The lower **Activity** panel shows tool-call history for the selected agent pane,
written by the agent's `PostToolUse` hook. It remains visible below Git.
...
Git and Activity are both visible at the same time. `Shift+Tab` changes which
panel receives scrolling input, and clicking either panel title selects it
directly.
